...
/**
 * Open broadcast API for automation apps (Tasker/MacroDroid/etc).
 * Processing is gated by [ExternalControlStore].
 */
class ExternalControlReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent?) {
        val action = intent?.action ?: return
        if (!ExternalControlStore(context).isEnabled()) return
        val serviceIntent = Intent(context, WalkieService::class.java).apply {
            this.action = action
