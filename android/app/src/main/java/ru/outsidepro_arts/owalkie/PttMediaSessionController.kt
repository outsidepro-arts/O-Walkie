...
/**
 * Routes headset / media play-pause events to PTT toggle when enabled.
 * Does not play audio; uses [PlaybackStateCompat.STATE_PAUSED] only to stay eligible for button delivery.
 */
class PttMediaSessionController(
    ...
    private fun dispatchToggle() {
        ...
        if (now - prev < DEBOUNCE_NS) return
        if (!lastToggleNs.compareAndSet(prev, now)) return
        mainHandler.post(onMediaButtonToggle)
    }

    companion object {
        private const val SESSION_TAG = "OWalkiePtt"
